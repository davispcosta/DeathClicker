...
function scene:create( event )
   ...
   local title = display.newText( "UPGRADES", 70, -20, "twcm", 20 )
   ...
   local scrollView = widget.newScrollView{
    ...
    for i=1, base.numUpgrades do
        ...
        local name = display.newText(base.upgrades[i].name, 0, 0, "twcm", 17)
        name.x = 150
        name.y = yOffset - 30
        name:setFillColor(0,0,0)
        scrollGroup:insert(name)

        local managerFunction = display.newText(base.upgrades[i].upgradeFuction, 0, 0,"twcm", 17)
        ...
        local buyBtnText = display.newText(loadedBase.areas[1].attributes[i].price, 0, 0, "twcm", 17)
